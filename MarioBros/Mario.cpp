...
void CMario::OnCollisionWith(LPCOLLISIONEVENT e, DWORD dt)
{
	if (e->ny != 0 && e->obj->IsBlocking())
	{
		...
		{
			isOnPlatform = true;
			CGame::GetInstance()->pipeX = e->obj->GetX();
		}
	}
	else
...
void CMario::HandleMarioHoldingKoopas()
{
	if (isHoldingKoopas)
	{
		...
		{
			koopasHold->SetY(koopasHold->GetY() - ((KOOPAS_BBOX_HEIGHT - MARIO_BIG_BBOX_HEIGHT) / 2));
			isHoldingKoopas = false;
			HandleMarioIsAttacked();
		}
	}
...
void CMario::HandleMarioGoOutsiteMap()
{
	if (x - MARIO_BIG_BBOX_WIDTH / 2 <= 0)
		x = MARIO_BIG_BBOX_WIDTH / 2;
	else if (x + MARIO_BIG_BBOX_WIDTH / 2 >= 2816)
	{
		x = 2816 - MARIO_BIG_BBOX_WIDTH / 2;
	}
